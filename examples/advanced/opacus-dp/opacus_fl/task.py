...
def load_data(partition_id: int, num_partitions: int):
    ...
    Uses torchvision.datasets.CIFAR10 instead of HuggingFace FederatedDataset
    for faster downloads on HPC clusters.
    """
...
def train(net, train_loader, privacy_engine, optimizer, target_delta, device, epochs=1):
    """Train the model with differential privacy using Opacus."""
    criterion = torch.nn.CrossEntropyLoss()
    net.to(device)
    net.train()
    for _ in range(epochs):
        for images, labels in tqdm(train_loader, "Training"):
            optimizer.zero_grad()
